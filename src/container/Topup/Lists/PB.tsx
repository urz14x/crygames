...
const PB: React.FC = () => {
  ...
  function selectHandler(event: React.MouseEvent<HTMLElement>) {
    ...
    const cashs = event.currentTarget.textContent;
    const toNumber = Number(cashs?.replace(" Cash", ""));
    ListCash.map((list) => {
      if (list.cash === toNumber) {
        setSelected(toNumber);
        setActive(true);
    ...
    <section className=" pt-10 min-h-screen">
      ...
      <div className=" flex justify-center items-center flex-col antialiased mt-12 space-y-5">
        ...
        <div className=" w-80 h-44 p-3 rounded-md  ">
            ...
            {ListCash.map((list, id: number) => {
              return (
                <button
                  ...
                  type="button"
                  className={` ${
                    selected === list.cash ? "ring ring-[#4E04E0]" : ""
                  } flex py-2 items-center justify-center gap-1 border-2 border-gray-800 rounded-md font-cg_font_primary_bold text-cg_color_primary antialiased hover:ring-1 hover:ring-[#4E04E0] cursor-pointer transition ease-in-out`}
                >
                  <svg
                    width="18"
                    height="19"
          ...
          <button
            ref={confirmBtn}
            className={`flex gap-3 items-center ml-3 px-4 py-2  rounded-md relative bottom-0 font-cg_font_primary_regular w-1/2 sm:w-auto ${
              active
