...
const PB: React.FC = () => {
  ...
  function selectHandler(event: React.MouseEvent<HTMLElement>) {
    ...
    const cashList = Number(cashs?.replace(" Cash", ""));
    ListCash.map((list) => {
      if (list.cash === cashList) {
        setSelected(cashList);
        setActive(true);
        setActive(cashList);
      }
    });
  ...
  function confirmHandler() {
    location.assign(
      `https://api.whatsapp.com/send?phone=6281223831901&text=Hallo%20saya%20ingin%20membeli%20voucher%20PB%0ANominal%20%3A%20${selected}%20Cash%0APembayaran%20%3A%20Dana`
    );
  }
